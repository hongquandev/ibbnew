...
include_once 'inc/homepage_slides.class.php';
$message = '';
...
switch ($action) {
    case 'add':
    case 'edit':
        $form_action = ($action == 'add') ? '?module=homepage_slides&action=add' : '?module=homepage_slides&action=edit&id=' . $_GET['id'];
        $form_action .= '&token=' . $token;
        include 'inc/admin.homepage_slides.add.php';
...
//bar
$url = array('add' => '?module=homepage_slides&action=add&token=' . $token,
    'list' => '?module=homepage_slides&action=list&token=' . $token);
