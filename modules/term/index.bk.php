...
switch ($action){
    case 'view':
        ...
        if (isSubmit() && $property_id != 0){
            if (isset($_SESSION['agent']) && is_array($_SESSION['agent']) and $_SESSION['agent']['id'] > 0){
                $data = $_POST;
                if($data['is_save_application'] == 1){
                    $files_ar = array();
                    $bid_term_cls->updateFormApplication($_SESSION['agent']['id'],$data,file);
                }
                print_r($_POST);
                print_r($_FILES);
                die();
                ...
                sendMailToAgent($property_id,$_SESSION['agent']['id'],$directory,$filename);
...
function sendMailToAgent($property_id = 0,$bidder_id = 0,$path = '',$filename = ''){
    ...
    if (strlen($remember) && $remember == 1) {
        $bid_term_cls->addNew($bidder_id, $agentInfo['agent_id'],$path.$filename);
    }
    //bidder_info
    $agent = $agent_cls->getCRow(array('firstname', 'lastname', 'email_address'), 'agent_id = ' . $bidder_id);
    ...
    $file = '';
    if (strlen($filename)){
        $file = array('path' => ROOTPATH.$path.$filename,
                  'filename' => $filename);
    }
