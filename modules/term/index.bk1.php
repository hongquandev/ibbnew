...
switch ($action) {
    ...
    case 'view-application':
        ...
        $smarty->assign('later_link', ROOTURL . '/?module=term&action=view&pid=' . $property_id);
        if (isset($_SESSION['agent'])    && is_array($_SESSION['agent']) and $_SESSION['agent']['id'] > 0) {
            $row = $bid_term_cls->getRow('bidder_id = ' . $_SESSION['agent']['id'] . '');
            if (is_array($row) and count($row) > 0) {
        ...
        if (isSubmit() && $property_id != 0) {
            if (isset($_SESSION['agent']) && is_array($_SESSION['agent']) and $_SESSION['agent']['id'] > 0) {
                ...
                $save = $bid_term_cls->addApplicationForm($_SESSION['agent']['id'], $file_application);
                if ($save) {
                    $_SESSION['rental_form_message'] = 'The rental application form has been saved successful.';
                }
            }
            if (getPost('is_save_application', 0) == 1) {
                redirect(ROOTURL . '/?module=term&action=view&pid=' . $property_id . '#rental_application_supporting');
            } else {
                redirect(ROOTURL . '/?module=term&action=view&pid=' . $property_id . '');
            }
        }
        break;
    default:
