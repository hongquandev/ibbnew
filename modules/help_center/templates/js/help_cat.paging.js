...
Ext.onReady(function(){
    ...
    store = new Ext.data.JsonStore({
        ...
        remoteSort: true,
        fields: [
            'catID',  'title' , 'active' , 'position', 'Edit', 'Delete', 'View', 'count','permission','description'
        ],
   ...
   /* store.setDefaultSort('position', 'DESC');*/
    ...
    function renderStatus(val, i, rec){
        ...
        if (rec.data.active == 1) {
            str = '<a class="grid_default" href = "javascript:void(0)" onclick="outAction(\'change-status\','+rec.data.catID+')">Active</a>';
        } else {
            str = '<a class="grid_warn_" href = "javascript:void(0)" onclick="outAction(\'change-status\','+rec.data.catID+')">InActive</a>';
        }
        return str;
    }
    ...
    var grid = new Ext.grid.GridPanel({
       ...
       tbar : [{
			     ...
			     cls : 'x-btn-text-icon',
				 handler : function() {
				    document.location = session.url_link.replace('[1]','add-cat');
			     }
				},'-',
                {
