...
Ext.onReady(function(){
    ...
    store = new Ext.data.JsonStore({
        ...
        remoteSort: true,
        fields: [
            'press_id',  'title' , 'short_content' , 'creation_date' , 'modified_date','active','cat_name','cat_id','show_date','show','url'
        ],
        proxy: new Ext.data.HttpProxy({
             url: session.action_link.replace('[1]','action=list-article')
        })
    });
    store.setDefaultSort('press_id', 'DESC');
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        viewConfig: {
            ...
            getRowClass : function(rec, rowIndex, p, store){
				...
				str += rec.data.short_content;
				str += '</div>';

				str += '<p class="grid_expand2" style="margin-left:75px">';
                str += '<a href = "'+session.url_link.replace('[1]','edit-article&id='+rec.data.press_id)+'">Edit</a>';
                str += '<span>|</span>';
				str += '<a href = "javascript:void(0)" onclick="outAction(\'delete\','+rec.data.press_id+')">Delete</a>';
                str += '<span>|</span>';
				if (rec.data.active == 1) {
                   str += '<a href = "javascript:void(0)" onclick="outAction(\'change-status\','+rec.data.press_id+')">Active</a>';
                } else {
                   str += '<a class="grid_warn" href = "javascript:void(0)" onclick="outAction(\'change-status\','+rec.data.press_id+')">InActive</a>';
                }
                str += '<span>|</span>';
                str += '<a href="'+rec.data.url+'" target="_blank">View</a>';
        ...
        tbar : [{
					...
					cls : 'x-btn-text-icon',
					handler : function() {
						document.location = session.url_link.replace('[1]','add-article')
					}
				},'-',
				{
					text : 'Delete',
					...
					handler : function() {
						...
						if (sm.hasSelection()){
							...
							for (i = 0; i < rows.length; i++){
								id_ar.push(rows[i].data.press_id);
							}
							if (id_ar.length > 0) {
