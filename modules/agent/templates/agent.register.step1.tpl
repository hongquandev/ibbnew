                    ...
                    <li class="wide">
                        <div>
                            <label>
                                ...
                                <br/>
                                <small style="font-weight: bold; font-style: italic;">{localize translate="(Please provide mobile number to enable sms notification & updates)"} </small>
                            </label>
                            <div class="input-box">
                ...
                {if $register_kind == 'transact'}
                    {if $transact_step == 1}
                        <button class="btn-blue-transact" onclick="goToTransact('#frmAgent',2)" style="float:left">
                            <span><span>{localize translate="Register User Account Only"}</span></span>
                        </button>
                        <div class="clearthis"></div>
                        <button class="btn-green-transact" onclick="goToTransact('#frmAgent',2)" style="float:left">
                            <span><span>{localize translate="Complete Application to transact"}</span></span>
                        </button>
                    {/if}
                    {if $transact_step >= 2}
...
<script type="text/javascript">
    ...
    function goToTransact(frm,transact_step){
        ...
        if(validation.isValid()){
            var isSubmit = true;
            //VALID
            if(jQuery('#checked_term',frm).attr('checked') != 'checked'){
                jQuery('#notify_'+jQuery('#checked_term',frm).attr('id')).addClass('check-validation-fail');
                isSubmit = false;
            }else{
                jQuery('#notify_'+jQuery('#checked_term',frm).attr('id')).removeClass('check-validation-fail');
            }
            ...
            if(jQuery('#email_address',frmAgent).parent().find('.validation-note').length == 1){
                isSubmit = false;
            }
            if(isSubmit){
