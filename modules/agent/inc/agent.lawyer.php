...
$row = $agent_lawyer_cls->getRow('SELECT al.*,
				...
				FROM '.$agent_lawyer_cls->getTable().' AS al WHERE al.agent_id = '.$_SESSION['agent']['id'],true);
...
//		}
        ...
        if (!$error){
			...
			foreach ($_POST['field'] as $key => $val) {
				if (isset($form_data[$key])) {
					$form_data[$key] = $agent_cls->escape($val);
				}
			}
			
			$row = $agent_lawyer_cls->getRow('agent_id = '.$_SESSION['agent']['id']);
			if (is_array($row) && count($row) > 0) {
				...
				$agent_lawyer_cls->update($form_data,'agent_id = '.$_SESSION['agent']['id']);	
				...
				$agent_lawyer_cls->insert($form_data);
			}
