...
$row = $agent_site_cls->getRow('agent_id = ' . $_SESSION['agent']['id'].' AND type = \'agency\'');
if (is_array($row) and count($row)> 0){
    $form_data = $row;
}
$background = $background_cls->getFields();
$form_data = $form_data + $background;
$rows = $background_cls->getRows('agent_id = '.$_SESSION['agent']['id']);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($form_data as $k=>$val){
        if(isset($_POST['fields'][$k])){
            if(!is_array($_POST['fields'][$k])){
                $form_data[$k] = $data[$k] = $agent_site_cls->escape($_POST['fields'][$k]);
            }else{
                $form_data[$k] = $data[$k] = $_POST['fields'][$k];
            }
        }
    }
    ...
    $form_data['logo'] = $data['logo'];
    if (!$error){
        if ($data['name'] || $data['logo'] != '' || $data['background_logo'] != ''){
            if ($data['name'] == ''){unset($data['name']);}
            $check = Agent_checkExitsSite($_SESSION['agent']['id'],'agency');
            if ( $check > 0){
    ...
    if (isset($_SESSION['agent']['bg']) and count($_SESSION['agent']['bg']) > 0){
        foreach ($_SESSION['agent']['bg'] as $k=>$val){
            $form_data['background_'.$k] = ($k == 'top')?MEDIAURL.'/store/uploads/background/img/'.$val:MEDIAURL.'/store/uploads/background/thumbs/'.$val;
            $row = $background_cls->getRow('agent_id = '.$_SESSION['agent']['id']." AND type = '{$k}'");
            $save_data['link'] = $val;
            $save_data['agent_id'] = $_SESSION['agent']['id'];
            $save_data['type'] = $k;
            if ($k == 'top'){
                $save_data['background_color'] = $data['background_logo'];
            }
            if (is_array($row) and count($row)> 0){
                $background_cls->update($save_data,"agent_id = {$_SESSION['agent']['id']} AND type = '{$k}'");
            }else{
        ...
        $rows = $background_cls->getRows('agent_id = '.$_SESSION['agent']['id']." AND type != 'top'");
        if (is_array($rows) and count($rows) > 0){
            $background_cls->update($save_data,"agent_id = {$_SESSION['agent']['id']}");
        }else{
            $save_data['agent_id'] = $_SESSION['agent']['id'];
            $save_data['type'] = 'left';
            $background_cls->insert($save_data);
