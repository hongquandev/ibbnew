...
<script type="text/javascript">
    ...
    var token = "{$token}";
    var show = "{$show}";
        session.action_link = '/modules/cms/action.admin.php?[1]&token={$token}';
        {literal}
        if (show == 'full') {
            session.action_link = '/modules/cms/action.admin.php?[1]&show=full&token='+ token;
        }
        {/literal}
		session.add_link = '?module=cms&action=edit&token={$token}';
        session.url_link = '?[1]&token={$token}';
		session.action_type = 'page';
		session.token = '{$token}';
		session.grid_title = 'Page List';
</script>
<script type="text/javascript" src="js/ext-base.js"></script>
<script type="text/javascript" src="js/ext-all.js"></script>
<script type="text/javascript" src="/modules/cms/templates/js/paging.en.js"></script>
