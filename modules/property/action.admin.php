...
if (eregi('-property', $action)) {
    ...
    switch ($action) {
        ...
        case 'delete-property-focus':
        case 'delete-property-home':
        case 'delete-property-inactive':
            if ($perm_ar['delete'] == 1) {
        ...
        case 'multidelete-property-focus':
        case 'multidelete-property-home':
        case 'multidelete-property-inactive':
            if ($perm_ar['delete'] == 1) {
        ...
        case 'active-property-focus':
        case 'active-property-home':
        case 'active-property-inactive':
            if ($perm_ar['edit'] == 1) {
                __propertyAttributeActionNew('active');
            } else {
                die(json_encode($perm_msg_ar['edit']));
            }
            break;
        case 'focus-property':
        case 'focus-property-focus':
        case 'focus-property-home':
        case 'focus-property-inactive':
            if ($perm_ar['edit'] == 1) {
            ...
            break;
        case 'set_jump-property':
        case 'set_jump-property-focus':
        case 'set_jump-property-home':
        case 'set_jump-property-inactive':
            if ($perm_ar['edit'] == 1) {
            ...
            break;
        case 'agent_active-property':
        case 'agent_active-property-focus':
        case 'agent_active-property-home':
        case 'agent_active-property-inactive':
            if ($perm_ar['edit'] == 1) {
                __propertyAttributeActionNew('agent_active');
            } else {
                die(json_encode($perm_msg_ar['edit']));
            }
            break;
        case 'validate-property':
        case 'validate-property-focus':
        case 'validate-property-home':
        case 'validate-property-inactive':
            __propertyValidateAction();
            break;
        case 'multiactive-property':
        case 'multiactive-property-focus':
        case 'multiactive-property-home':
        case 'multiactive-property-inactive':
            if ($perm_ar['edit'] == 1) {
            ...
            break;
        case 'change_pay-property':
        case 'multichange_pay-property':
            if ($perm_ar['edit'] == 1) {
                __propertyChangePay();
            } else {
                die(json_encode($perm_msg_ar['edit']));
            ...
            break;
        case 'change_reastatus-property':
        case 'multichange_reastatus-property':
            if ($perm_ar['edit'] == 1) {
                __propertyChangeReaStatus();
            } else {
                die(json_encode($perm_msg_ar['edit']));
            }
            break;
...
function __mediaAction()
{
    global $token, $action, $media_cls, $property_media_cls, $property_cls, $package_cls, $config_cls;
    $property_id = (int)restrictArgs(getQuery('property_id', 0));
    $agent_id = (int)$_SESSION['admin']['agent']['id'];
    $path = $_SESSION['admin']['property']['path'];
    $path_relative = '/store/uploads/' . $agent_id . '/' . $property_id;
        ...
        switch ($action) {
            case 'upload-media':
                ...
                if ($type == 'photo') {
                    ...
                    if (isset($row['video_size']) && strlen($row['video_size'])) {
                        $sizeLimit = $row['video_size'] * 1024 * 1024;
                    }
                    $allowedExtensions = array('flv', 'wmv', 'mp4');
                    $path_pre = $path_relative . '/';
...
function __propertyListAction()
{
    ...
    if ($key != '' and $key != 'all') {
        switch ($key) {
            ...
            case 'forthcoming':
                ...
                break;
            case 'sale':
                $filter = "pro.auction_sale = '" . $auction_arr['private_sale'] . "'";
                break;
            case 'ebidda-live':
    ...
    if (is_array($rows) and count($rows) > 0) {
        ...
        foreach ($rows as $row) {
            ...
            $row['reaxml_status'] = ucwords(PE_getPropertyStatusREA_xml($row['property_id']));
            $row['end_time_tr'] = $row['end_time'];
            $row['start_time_tr'] = $row['start_time'];
            $dt = new DateTime($row['end_time']);
            $row['end_time'] = $dt->format('Y/m/d');
            /**/
            if ($row['start_time'] == '5000-05-05 00:00:00') {
                $row['start_time'] = 'For ' . (PE_isRentProperty($row['property_id']) ? 'Rent' : 'Sale') . ' - no auction scheduled';
            } else {
                if ($row['start_time'] > '0000-00-00 00:00:00') {
                    $dt = new DateTime($row['start_time']);
                    $row['start_time'] = $dt->format('Y/m/d, g:i a');
                } else {
                    $row['start_time'] = '0000-00-00';
                }
            }
...
function sendMailPostedToLive($property_id)
{
    try {
        global $config_cls, $property_provider_email_cls, $log_cls, $property_cls;
        /* Check a property had been posted to live*/
        if (PE_isLiveProperty($property_id) && $property_id > 0) {
            //Update posted_to_live_time
            $property_cls->update(array('posted_to_live_time' => date('Y-m-d H:i:s')), 'property_id = ' . $property_id);
            $agent = PE_getAgent(0, $property_id);
            $link = ROOTURL . '/?module=property&action=view-auction-detail&id=' . $property_id;
            $link = '<a href="' . $link . '">' . $link . '</a> ';
            $msg = $config_cls->getKey('email_vendor_property_posted_to_live_msg');
            $subject = $config_cls->getKey('email_vendor_property_posted_to_live_subject');
            $subject = str_replace('[ID]', $property_id, $subject);
            $msg = str_replace(array('[ID]', '[link]', '[vendor_name]'), array($property_id, $link, $agent['firstname'] . ' ' . $agent['lastname']), $msg);
            $from = $config_cls->getKey('general_contact_email');
            $to = array($agent['email_address'], $config_cls->getKey('general_service_provider_email'));
            sendEmail_func($from, $to, $msg, $subject);
            $action = "Posted to live";
            ...
            $emailto = (!empty($row['email']) && strlen($row['email']) > 0) ? $row['email'] : ((strlen($general_service_provider_email) > 0) ? $general_service_provider_email : $from);
            $msg = $config_cls->getKey('email_service_provider_msg');
            $msg = str_replace(array('[ID]', '[ACTION]'), array($property_id, $action), $msg);
            $subject = $config_cls->getKey('email_service_provider_msg_subject');
            $subject = str_replace(array('[ID]'), array($property_id), $subject);
            sendEmail_func($from, $emailto, $msg, $subject);
...
function __propertyAttributeAction($attr = '')
{
    ...
    if ($property_id > 0) {
        if ($row[$attr] == 0) {
            ...
            /*email to let them know their property has been posted to live*/
            sendMailPostedToLive($property_id);
            if ($attr == 'focus') {
...
function __propertyAttributeActionNew($attr = '')
{
    global $property_cls, $action;
    $result = array();
    $property_id = (int)restrictArgs(getParam('property_id', 0));
    ...
    if ($property_id > 0) {
        if ($row[$attr] == 0) {
            if ($attr == 'active' and $row['end_time'] < date('Y-m-d H:i:s') and $row['pay_status'] == Property::PAY_COMPLETE and $row['auction_sale'] == $auction_sale['auction']) {
                $result['msg'] = 'This property had been stopped, You can not active.';
                die(json_encode($result));
            }
            $property_cls->update(array($attr => 1), 'property_id = ' . $property_id);
            /*email to let them know their property has been posted to live*/
        ...
        $return_row = $property_cls->getRow('property_id = ' . $property_id);
        $result['property_id'] = $property_id;
        $result['property_data'] = $return_row;
    }
    /*
    $result = array('property_id' => $property_id,
                    'status' => (int)$row[$attr ],
                    'action' => $attr );
    */
    $result['msg'] = 'Datas have been updated.';
    die(json_encode($result));
}
...
function __propertyChangeReaStatus()
{
    global $property_cls, $systemlog_cls;
    $property_ids = getParam('property_id');
    $new_status = getParam('status');
    $endtime_ar = getParam('endtime_ar');
    $starttime_ar = getParam('starttime_ar');
    //print_r($endtime_ar);
    //print_r($starttime_ar);
    //die();
    $message = '';
    if (strlen($new_status) > 0 && strlen($property_ids) > 0) {
        ...
        - offmarket
        */
        /*Not sure we should set any restrictions on a user setting of status?
·         My thinking is
·         Current is a live property and can change to anything
·         Sold / Leased is automatic when an auction is complete and is above reserve
·         Passed in is automatic  when an auction is complete and is below reserve
·         Under offer is automatic when an offer is made and accepted

·         The normal flow would be current, on the market, sold,    or current, under offer, sold
·         Sold or leased needs to change to current or under offer, withdrawn or offmarket
·         Passed in needs to change back to current, or under offer, withdrawn or offmarket to then sold/leased
·         Withdrawn/offmarket are manual status settings
         * */
        ...
        $systemlog_cls->insert(array('Updated' => date("Y-m-d H:i:s"),
            'Action' => 'UPDATE',
            'Detail' => $detail . $property_ids . " SET STATUS: " . $new_status,
            'UserID' => $_SESSION['Admin']['EmailAddress'],
            'IPAddress' => $_SERVER['REMOTE_ADDR']));
        $message = 'Success';
    }
