...
function showAdvertisedPrice($property_id = 0)
{
    ...
    $row = $property_cls->getCRow(array('advertised_price_from', 'advertised_price_to', 'price_view'), 'property_id = ' . $property_id);
    if (is_array($row) and count($row) > 0) {
        if(!empty($row['price_view'])){
...
{
    global $region_cls, $property_entity_option_cls, $property_cls, $config_cls, $agent_cls;
    $wh_str = 'property_id = ' . $property_id;
    $review = array();
    if ($property_id > 0) {
        $sql = 'SELECT SQL_CALC_FOUND_ROWS
        ...
        if ($row['document_ids'] != '') {
            ...
            if (is_array($rows) && count($rows) > 0) {
                foreach ($rows as $key => $row) {
                    ...
                    $file_name_ar = explode('/',$rows[$key]['file_name']);
                    $rows[$key]['file_name'] = $file_name_ar[count($file_name_ar) -1];
                }
                return $rows;
            }
        }
 ...
 * @ output :
 **/
function Property_getList($wh_clause = '', $p = 1, $len = 1, $search_query = '', $pag_link = '', $is_mime_list = false)
{
    global $smarty
           , $property_cls
           , $region_cls
    ...
    $show_clause = " pro.active = 1 AND pro.agent_active = 1 AND pro.pay_status = " . Property::CAN_SHOW;
    if($is_mime_list){
        $show_clause = " 1";
    }
    ...
    $pag_url = $pag_link . $url_search . $url_part;
    $pag_cls->setTotal($total_row)
        ->setPerPage($len)
        ->setCurPage($p)
        ->setLenPage(10)
    ...
    if (is_array($rows) and count($rows) > 0) {
        foreach ($rows as $key => $row) {
            ...
            $data[$key]['release_time'] = $dt_release->format('d F Y g.ia');
            }else{
            unset($data[$key]['release_time']);
            }
            $data[$key]['is_release_time'] = date('Y-m-d H:i:s') > $row['release_time'];
    ...
    try {
        ...
        if (count($rows) > 0 and is_array($rows)) {
            foreach ($rows as $key => $row) {
                ...
                //'
                //row['bid_price'] = str_replace(".",",", $row['bid_price']);
                $agent_manager_id = PE_getManager($row['property_id']);
                $row['agent_manager_name'] = A_getFullName($agent_manager_id);
                /*---------------- CONTENT----------------- */
                $content['No'][] = $key + 1;
                ...
                $content['Status'][$key] = PE_getPropertyStatusREA_xml($row['property_id']);
                ...
                $content['Agent Name'][$key] =  $row['agent_manager_name'];
    ...
    if ($property_id > 0) {
        ...
        if ($property_cls->hasError()) {
            ...
            if ((($row['price'] < $reserve_price
                    || (Bid_isLastBidVendor($row['property_id']) && $row['price'] >= $reserve_price)
                    || ($row['price'] >= $reserve_price && $row['check_start']))
                    && $start_price > 0
                    && $reserve_price > 0
                    && ($row['stop_bid'] == 1 || ($row['end_time'] > '0000-00-00 00:00:00' && $row['end_time'] < date('Y-m-d H:i:s')))
                )OR ($row['ofAgent'] && $row['stop_bid'] == 1 && $start_price > 0 && $reserve_price > 0 && $row['end_time'] > '0000-00-00 00:00:00')
            ) {
...
function Property_getDetail($id)
{
    ...
    if ($property_cls->hasError()) {
        ...
        if($row['release_time'] > '0000-00-00 00:00:00'){
        $dt_release = new DateTime($row['release_time']);
        $data['info']['release_time'] = $dt_release->format('d F Y g.ia');
        }else{
        unset($data['info']['release_time']);
        }
        $data['info']['is_release_time'] = date('Y-m-d H:i:s') > $row['release_time'];
...
function updateSlugProperty($property_id = 0)
{
    global $property_cls, $region_cls;
    if ($property_id <= 0) {
        return false;
    }
    $rows = $property_cls->getRows('SELECT SQL_CALC_FOUND_ROWS
                            pro.property_id,
                            pro.address,
                            pro.suburb,
                            pro.postcode,
    ...
    if (is_array($rows) && count($rows) > 0) {
        foreach ($rows as $row) {
            $address = $row['address'] . ' ' . $row['suburb'] . ' ' . $row['state_code'] . ' ' . $row['postcode'] . ' ';
            $slug = strtolower(
                preg_replace(
                    array(
                        '/[^a-zA-Z0-9 -]/',
            ...
            if (!empty($slug)) {
                $property_cls->update(array('slug' => $slug), ' property_id = "' . $row['property_id'] . '" ');
            }
        }
    }
}
