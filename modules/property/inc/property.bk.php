        ...
        foreach ($rows as $key => $row) {
            ...
            if ($row['auction_sale'] != $auction_sale_ar['private_sale']) {
                if ($row['start_time'] == '5000-05-05 00:00:00' || $row['start_time'] == '0000-00-00 00:00:00') {
                    ...
                    $data[$key]['start_time'] = $dt->format($config_cls->getKey('general_date_format') . ', g:i a');
                }
...
function Property_getDetail($id)
{
    ...
    if ($property_cls->hasError()) {
        ...
        if ($data['info']['isBlock'] || $data['info']['ofAgent']) {
            $data['info']['no_more_bids'] = PE_isNoMoreBids($row['property_id'], isset($_SESSION['agent'])
                ? $_SESSION['agent']['id']
                : 0);
        ...
        if ($row['auction_sale'] != $auction_sale_ar['private_sale']
            ...
            $dt = new DateTime($row['end_time']);
            if ($row['end_time'] == '5000-06-06 00:00:00') {
                $data['info']['end_time'] = 'For ' . (PE_isRentProperty($row['property_id']) ? 'Rent' : 'Sale') . '';
            } else {
                $data['info']['end_time'] = $dt->format($config_cls->getKey('general_date_format'));
            }
            $dt1 = new DateTime($row['start_time']);
            if ($row['start_time'] == '5000-05-05 00:00:00') {
                $data['info']['start_time'] = 'For ' . (PE_isRentProperty($row['property_id']) ? 'Rent' : 'Sale') . '';
            } else {
                $data['info']['start_time'] = 'AUCTION STARTS : ' . $dt1->format($config_cls->getKey('general_date_format') . ' @ g:i');
            }
            $current_time = new DateTime(date('Y-m-d H:i:s'));
            if($row['start_time'] == '5000-05-05 00:00:00' && $row['end_time'] == '5000-06-06 00:00:00'){
                ...
                $data['info']['title'] = 'AUCTION STARTS : ' . $dt1->format($config_cls->getKey('general_date_format') . ' @ g:i');
                ...
                if ($row['end_time'] < date('Y-m-d H:i:s')) { // END AUCTION
                    ...
                    $data['info']['title'] = Localizer::translate('AUCTION ENDS') . ': <span class="countdown-cls-'.$row['property_id'].'"></span>';
