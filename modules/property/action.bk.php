        ...
        if ($sending == 'agent') {
            ...
            <table style="width: 100%;font-size: 12px">
                <tbody>
                    ...
                    <tr><td colspan="2"><br/><br/><br/><span style="color:#3a47ff"><i><span style="text-decoration: underline">BidRhino NOTE</span>: This notification should be sent to the Agents email address and their bidRhino,
                      my messages inbox. If they have an SMS notification, they should receive this as an SMS also. (Currently only sent on my messages inbox)</i></span></td></tr>
                </tbody>
            </table>';
        } else { // Buyer
            $message = '
            <table style="width: 100%;font-size: 12px">
                <tbody>
                    ...
                    <tr><td colspan="2"><br/><br/><br/><span style="color:#3a47ff"><i><span style="text-decoration: underline"> BidRhino NOTE:</span>This notification should be sent to the Users email address and their bidRhino,
                      my messages outbox. If they have an SMS notification, they should receive this as an SMS also. (Currently sent to users email and my messages outbox)</i></span></td></tr>
                </tbody>
            </table>';
        }
    }
    ...
    if ($property_id > 0) {
        ...
        if (count($row_vendor) > 0 and is_array($row_vendor)) {
            ...
            $content = '<label style="font-size: 16px; color: #2f2f2f;"> Dear '.$vendor_name.',</label>
						<p style="font-size:14px; margin-bottom: 5px; color: #222222;">
						    A Buyer wants to '.($isRentProperty == true ? 'RENT NOW' : 'BUY NOW').' your property ID#'.$property_id.' with '.showPrice($buynow_price).'
						</p>
						<p  style="font-size:14px;margin-top: 15px;color: #222222;">
							Please login to your account and follow link <a href="'.$link_offer.'">Property Offers</a> to Accept or Cancel.
						</p>
						<p  style="font-size:14px;margin-top: 15px;color: #222222;">
							<strong style="width:50px;color: #222222;">Thank you</strong>
						</p>
						</br>
						';
            ...
            $params_email_vendor['sms_content'] =  'A Buyer wants to '.($isRentProperty == true ? 'RENT NOW' : 'BUY NOW').' your property ID#'.$property_id.' with '.showPrice($buynow_price);
        ...
        $content = '<label style="font-size: 16px; color: #2f2f2f;"> Dear '.$buyer_name.',</label>
                    <p style="font-size:14px; margin-bottom: 5px; color: #222222;">
                        You have sent a '.($isRentProperty == true ? 'RENT NOW' : 'BUY NOW').' with property ID#'.$property_id.' with '.showPrice($buynow_price).' successful.
                    </p>
                    <p  style="font-size:14px;margin-top: 15px;color: #222222;">
                        <strong style="width:50px;color: #222222;">Thank you</strong>
                    </p>
...
function __makeAnOfferRefuseAction() {
    ...
    if ($message_id > 0) {
        ...
        if (count($rows_mess) > 0 and is_array($rows_mess)) {
            ...
            sendNotificationByEventKey('user_refused_offer_vendor',$params_email_vendor);
        }
    }
