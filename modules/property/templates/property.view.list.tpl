        ...
        <ul class="locat-list">
            {if isset($property_data) and is_array($property_data) and count($property_data) > 0}
                ...
                {foreach from = $property_data key = k1 item = row}
                    <li class="property-list-item">
                        ...
                        {include file = "`$ROOTPATH`/modules/property/templates/property.list.box.tpl"}
            ...
            {else}
                {if in_array($action,array('view-live_agent-list','view-forthcoming_agent-list'))}
                    <div class="message-box message-box-add" style="width: 608px;text-align:center">
                        <i>Sorry, but there are no properties available based on your selection. Please modify the
                            filters to search again. Thanks!</i>
                    </div>
                {else}
                    <div class="message-box message-box-add" style="width: 608px;">
                        <center><i>Sorry, but there are no properties available based on your selection. Please modify
                                the filters to search again. Thanks!</i></center>
                    </div>
                {/if}
            {/if}
        </ul>
...
{if in_array($action,array('view-auction-list','view-sale-list','search','search-auction','search-sale','view-forthcoming-list',
'view-live_agent-list','view-forthcoming_agent-list','search-ebiddar','search-agent-auction'))}
    <script type="text/javascript">
        {literal}
        jQuery(function () {
            updateLastBidder();
    ...
    <script type="text/javascript">
        {literal}
        $(function () {
            jQuery('.btn-bid-green').tipsy({
                gravity: $.fn.tipsy.autoNS,
                fallback: " <div style='padding: 5px;text-align: justify'>When the bid button turns green the property is on the market</div>",
                html: true
            });
            jQuery('.btn-bid-vendor-green').tipsy({
                gravity: $.fn.tipsy.autoNS,
                fallback: " <div style='padding: 5px;text-align: justify'>When the bid button turns green the property is on the market</div>",
                html: true
            });
