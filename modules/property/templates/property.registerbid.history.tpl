...
{/literal}
<div id="register-Agent" class="myaccount ma-messages mb-20px bid-history-dt" style="overflow-y: scroll;max-height: 270px;">
{* BEGIN tabs*}
<div id="container-register-bid" class="menu-register-bid">
<ul class="menu-register">
    {if isset($auctionSaleCode) && $auctionSaleCode != 'private_sale'}
    ...
    {*<li id="ActualBidReport" rel="actual-bid-report">Actual Bids Report</li>*}
        <li id="RegToBidReport" rel="reg-to-bid-report" {if isset($auctionSaleCode) && $auctionSaleCode == 'private_sale'}class="active"{/if}>Register To {if isset($auctionSaleCode) && $auctionSaleCode == 'private_sale'}Offer{else}Bid{/if} Report</li>
            ...
            {if is_array($rows) and count($rows) > 0}
                ...
                {foreach from = $rows key = i item = row}
                    <tr id="tr_register_{$i+1}"
                        ...
                        <td><a class="name" href="javascript:void(0)">{if $row.is_disable == 1 || $row.allow == 0}
                        ...
                        <script type="text/javascript">
                            $('#allow_{/literal}{$row.ID}{literal}').bind('click',function(){
                                if (typeof bid_{/literal}{$property_id}{literal} === 'undefined'){
                                    var bid_{/literal}{$property_id}{literal} = new Bid();
                                    bid_{/literal}{$property_id}{literal}._options.property_id = {/literal}{$property_id}{literal};
                                }
...
{literal}
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            ...
            $('.menu-register li').click(function(){
                switch_tabs($(this));
            });
            switch_tabs($('.active'));
            function switch_tabs(obj) {
                var id = obj.attr("rel");
                $('#register-Agent .content').hide();
                $('.content-bid').hide();
                $('.menu-register li').removeClass("active");
                $('.' + id).show();
                obj.addClass("active");
            }
            console.log(document.location.href);
            if(document.location.href.indexOf('regtobidreport') > -1){
                jQuery('#RegToBidReport').click();
            }
        });
