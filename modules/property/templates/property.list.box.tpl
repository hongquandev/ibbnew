<ul class="property-list-item property-list-item-{$row.property_id}">
    <li class="property-i">
        ...
        <div class="i-info-left f-left">
            ...
            <div class="more-detail-info">
                ...
                <span class="detail-icons">
                    ...
                    {/if}
                    {if $row.land_size > 0}
                        <span style="color: #017db9" class="landsize">&nbsp; Land size {$row.land_size}</span>
                    {/if}
                    {if $row.frontage > 0}
                        <span style="color: #017db9" class="landsize">&nbsp; Frontage  {$row.frontage}m</span>
                    {/if}
                </span>
            </div>
        </div>
        ...
        <div class="f-left agent-list-info">
            <div class="agent-list-info-main">
                {if $row.agent && $row.agent.logo != ''}
                    <a href="{$row.agent.link}"><img src="{$MEDIAURL}/{$row.agent.logo}" style="max-width: 190px;width: 100%;height: auto;max-height: 190px" alt="{$MEDIAURL}/{$row.agent.company_name}" title="Click here to view all property"/></a>
                {/if}
            </div>
        ...
        {/if}
        {if !$row.is_release_time}
            {assign var = mao_str value = "return showMess('This property is not release. You can not offer.')"}
            {assign var = buynow_str value = "return showMess('This property is not release. You can not Buy/Rent Now.')"}
        {/if}
        ...
        <div class="status-panel">
            {if $row.release_time}
                <div class="status-time"><span class="label">{if $row.is_release_time}Released{else}Release{/if} Date: </span><span class="value">{$row.release_time}</span></div>
            {/if}
            <div class="status-list"><span class="label">Status: </span><span class="value">{$row.title_status}</span></div>
