		...
		listeners: {
            ...
            selectionchange: function(sm) {
                if (sm.getCount()) {
                    ...
                    Ext.getCmp('btnChangeReaStatus').enable();
					var l = sm.getCount();
					var s = l != 1 ? 's' : '';
                    ...
                    Ext.getCmp('btnChangeReaStatus').disable();
	...
	var grid = new Ext.grid.EditorGridPanel({
        ...
        columns : [
            sm,
            {
             ...
             renderer : renderEdit
                ...
                header: "Posted to live Time",
                dataIndex: 'posted_to_live_time',
                width: 100,
                align: 'left'
            } ,{
                header: "Start time",
            ...
            getRowClass : function(rec, rowIndex, p, store){
                ...
                if (rec.data.confirm_sold == 0) {
                    ...
                    if (rec.data.active == 1) {
                        str += '<a href = "javascript:void(0)" class="clk-to-active" data="active,' + rec.data.property_id + '" >Active</a>';
                    } else {
                        str += '<a class="grid_warn clk-to-active" data="active,' + rec.data.property_id + '" href = "javascript:void(0)" >InActive</a>';
                    }
                    str += '<span>|</span>';
                    ...
                    if (rec.data.agent_active == 1) {
                        ...
                        str += '<a href = "javascript:void(0)" class="clk-to-active" data="agent_active,' + rec.data.property_id + '" >Online</a>';
                    } else {
                        str += '<a class="grid_warn clk-to-active" href = "javascript:void(0)" data="agent_active,' + rec.data.property_id + '" >Offline</a>';
                    }
		...
		tbar : [{
                        ...
                        {text: 'Current',
                            handler: function () {
                                ...
                                if (sm.hasSelection()) {
                                    ...
                                    var endtime_ar = [];
                                    var starttime_ar = [];
                                    var endtime_new =  null;
                                    var starttime_new =  null;
                                    var rows = sm.getSelections();
                                    if(rows.length > 1){
                                        Ext.Msg.alert('Warning.', 'Please select only one property');
                                        return true;
                                    }

                                    for (var i = 0; i < rows.length; i++) {
                                        id_ar.push(rows[i].data.property_id);
                                        var propery_id = rows[i].data.property_id;
                                        var end_time = rows[i].data.end_time_tr;
                                        var start_time = rows[i].data.start_time_tr;
                                        var check = false;
                                        Ext.Msg.prompt('End time', 'Property ID#'+propery_id+': Please enter End time :', function(btn, text1){
                                            if (btn == 'ok'){
                                                // process text value and close...
                                                endtime_new = text1;
                                                Ext.Msg.prompt('Start time', 'Property ID#'+propery_id+': Please enter Start time :', function(btn, text2){
                                                    if (btn == 'ok'){
                                                        // process text value and close...
                                                        starttime_new = text2;
                                                        check = true;
                                                        console.log(endtime_new);
                                                        console.log(starttime_new);
                                                        console.log(id_ar);
                                                        changeReaStatus(id_ar, 'current',endtime_new,starttime_new);
                                                    }
                                                },'','',start_time);
                                            }
                                        },'','',end_time);
                                    ...
                                    Ext.Msg.alert('Warning.', 'Please chose one row before changing status.');
                                }
                            }
                        },
                        {text: 'Sold',
                        ...
                        {text: 'Passed In',
                            handler: function () {
                                ...
                                if (sm.hasSelection()) {
                                    ...
                                    changeReaStatus(id_ar, 'passed-in');
                                } else {
                                    Ext.Msg.alert('Warning.', 'Please chose one row before changing status.');
                                }
                            }
                        ...
                        {text: 'Under Offer',
                            handler: function () {
                                ...
                                if (sm.hasSelection()) {
                                    ...
                                    changeReaStatus(id_ar, 'underoffer');
                                } else {
                                    Ext.Msg.alert('Warning.', 'Please chose one row before changing status.');
                                }
                            }
                        ...
                        {text: 'Withdrawn',
                            handler: function () {
                                ...
                                if (sm.hasSelection()) {
                                    ...
                                    changeReaStatus(id_ar, 'withdrawn');
                                } else {
                                    Ext.Msg.alert('Warning.', 'Please chose one row before changing status.');
                                }
                            }
                        ...
                        {text: 'Offmarket',
                            handler: function () {
                                ...
                                if (sm.hasSelection()) {
                                    ...
                                    changeReaStatus(id_ar, 'offmarket');
                                } else {
                                    Ext.Msg.alert('Warning.', 'Please chose one row before changing status.');
                                }
                            }
				...
				},'-',
                {
                    text : 'Export CSV',
                    icon : '/admin/resources/images/default/dd/drop-yes.gif',
                    tooltip: 'Export property to Csv File',
                    cls : 'x-btn-text-icon',
                    disabled: true,
                    id:'btnExportCsv',
                    //ref: '../changeButton',
                    handler : function() {
                        ...
                        if (sm.hasSelection()){
                            ...
                            if (id_ar.length > 0) {
                                outAction('exportPropertyCSV',id_ar.join(','));
                            }
                        } else {
...
function ajaxActionNew(type,val,url) {
	...
	Ext.Ajax.request({
		...
		success : function ( result, request ) {
			var jsonData = Ext.util.JSON.decode(result.responseText);
            //alert(jsonData.property_id);
            console.log(jsonData.property_data);                        
			//var resultMessage = jsonData.data.result;
			hideWaitBox();
			store.reload({params:{key:Ext.getCmp('cmbFilter').getValue()}});                                                
			if (result.responseText && result.responseText.length > 0) {
            ...
            if(jsonData.property_data.active == 1 && jsonData.property_data.agent_active == 1) {
                //window.open('index.php?action=generatePDF&property_id='+jsonData.property_id,'PDF file');
                myPopup.location = 'index.php?action=generatePDF&property_id='+jsonData.property_id;
            } else {
                myPopup.close();
            }  
		},
...
function changeReaStatus(id_ar, value,endtime_ar,starttime_ar) {
    if (id_ar.length > 0) {
        var url = '../modules/property/action.admin.php?action=multichange_reastatus-property&token=' + session.token;
        showWaitBox();
        $.post(url, {status: value, property_id: id_ar.join(','), endtime_ar:endtime_ar, starttime_ar:starttime_ar}, function (data) {
            hideWaitBox();
            var result = jQuery.parseJSON(data);
            if (result == 'Success') {
                store.reload({params: {key: Ext.getCmp('cmbFilter').getValue()}});
...
jQuery(document).ready(function() {
    //alert('ready')
    jQuery('.clk-to-active').bind('click',function() {
        //alert('click');
        var data = jQuery(this).attr('data');
        var arr = data.split(',');
        var action = arr[0];
        var property_id = arr[1];         
        outActionNew(action, property_id);     
    });
});
