...
{if is_array($rows) and count($rows) > 0}
	{foreach from = $rows key = k item = row}
        <div style="margin:1px;background-color:#FFFBFB;border:1px solid #FFECEC;width:434px;padding-bottom: 33px;">
            <div style="margin:10px">
                {php}//$this->_tpl_vars['row']['content'] =  strip_tags($this->_tpl_vars['row']['content']);{/php}
                ...
                <div class="maogb_box2" id = "maogb_minibox_{$row.message_id}" style="display:none; width: 90%; margin-top: 4px;">
                    ...
                    {if $row.type == "forth_auction" AND $row.reserve > $row.offer_price}
                        <button class="btn-blue" onClick="acceptOfferPopup('{$row.message_id}','{$row.entity_id}','{$row.agent_id_from}','#money_{$row.message_id}')">
                            <span><span>Submit</span></span>
                         </button>
                    {else}
                         <button class="btn-blue" onClick="MAOGridBox.submit('{$row.message_id}','{$row.entity_id}','{$row.agent_id_from}','#money_{$row.message_id}')">
                            <span><span>Submit</span></span>
                         </button>
                    {/if}
                     <button class="btn-gray" onClick="MAOGridBox.cancel('{$row.message_id}','{$row.entity_id}')" style="width: 70px">
                        <span style="padding: 3px 10px 10px 24px"><span>Cancel</span></span>
                     </button>
                </div>
                <button class="btn-blue" onClick="MAOGridBox.accept('{$row.message_id}','{$row.entity_id}','{$row.agent_id_from}','#money_{$row.message_id}')">
                    <span><span>Accept</span></span>
                </button>
                <button class="btn-gray" onClick="MAOGridBox.refuse('{$row.message_id}','{$row.entity_id}')">
    ...
    <script type="text/javascript">
        ...
        function acceptOfferPopup(message_id,entity_id,agent_id_from,money_id){
            ...
            offerPopup.updateContainer('<div class="popup_container" style="width:295px;height: 150px;"><div id="contact-wrapper">\
                             <div class="title"><h2 id="txtt"> This page says:<span id="btnclosex" onclick="closeOfferPopup()">Close X</span></h2> </div>\
