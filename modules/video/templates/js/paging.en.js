...
Ext.onReady(function(){
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        sm,
        {
            id: 'video_id', // id assigned so we can apply custom css (e.g. .x-grid-col-topic b { color:#333 })
            header: "ID ",
            dataIndex: 'video_id',
            width: 40,
			align: 'center',
        ...
        plugins:[new Ext.ux.grid.Search({
					...
					,disableIndexes : ['video_content']
					,minChars : 3
					,autoFocus : true
					,position : 'top'
					,checkIndexes : ['video_id','video_name']
					,showSelectAll : true
					,width : 250
					,align : 'right'
		})]
					...
					handler : function() {
						...
						if (sm.hasSelection()){
							...
							for (i = 0; i < rows.length; i++){
								id_ar.push(rows[i].data.video_id);
							}
							if (id_ar.length > 0) {
        ...
        bbar : new Ext.PagingToolbar({
            ...
            store : store,
            displayInfo : true,
            displayMsg : 'Total Video page <div id="video-paging" style="display: none">{0} - {1} of </div> {2}',
            emptyMsg : "No Video page to display",
