...
function outAction(type,val){
	switch (type) {
        ...
        case 'exportPropertyCSV':
            document.location = location.origin +'?module=property&action=exportPropertyCSV&property_ids='+val;
            break;
        case 'exportCustomerCSV':
            document.location = location.origin +'?module=agent&action=exportCustomerCSV&agent_ids='+val;
            break;
		case 'delete':
			Ext.Msg.show({
				title :'Delete record?'
...
function outActionNew(type,val){
    ajaxActionNew(type,val,session.action_link.replace('[1]','action=' + type + '-' + session.action_type));
}
