    ...
    onExistEmail: function (data) {
        ...
        if (JSONobject.error == 0) {
            if (JSONobject.num_rows > 0) {
                ...
                jQuery(Common.effect_obj).parent().find('.validation-note').remove();
                jQuery(Common.effect_obj).parent().append('<span class="validation-note" style="color: #ff0000">This mail is existed!</span>');
            } else {
    ...
    warningObject: function (id_obj, turnoff) {
        if (typeof id_obj == 'string') {
            ...
            if (turnoff == true) {//
                ...
                //jQuery(id_obj).css({"border":"1px dashed #ff0000 !important","color":"#ff0000 !important"});
                var oldCss = jQuery(id_obj).attr('style');
                jQuery(id_obj).css("cssText", "border: 1px dashed #ff0000 !important;color:#ff0000 !important;" + oldCss);
                if (notify = jQuery(id_notify)) {
                    //jQuery(notify).css({"color":"#ff0000"});
                    jQuery(notify).css("cssText", "color:#ff0000 !important;" + jQuery(notify).attr('style'));
                }
            }
        }
    },
...
function showLoadingPopup() {
    ...
    loading_popup.updateContainer('<div class="popup_container" style="width: 214px;height: 138px"><img width="100%" height="auto" src="' + ROOTURL + '/modules/general/templates/images/processing.gif" alt="loading" /></div>');
...
function showLoadingPopup2() {
    ...
    loading_popup2.updateContainer('<div style="position:absolute;z-index: 999;width: 214px;height: 138px"><img width="100%" height="auto" src="' + ROOTURL + '/modules/general/templates/images/processing.gif" alt="loading" /></div>');
...
function showMess__(error_to_from, url, show_ok_btn) {
    ...
    mess_popup.updateContainer('<div class="popup_container" style="width:100%;max-width: 305px;height:auto;min-height: 120px;">\
             ...
             <span><img style="margin-right: 5px" src="/modules/property/templates/images/alert.png" >' + error_to_from + '</span></div>\
			 ' + btn_str + '\
			  </div></div></div>');
