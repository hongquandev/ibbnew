...
$.fn.visualize = function(options, container){
	return $(this).each(function(){
		...
		var o = $.extend({
			...
			colors: ['#be1e2d','#008000','#92d5ea','#ee8310','#8d10ee','#5a3b16','#26a4ed','#f45a90','#e9e744'],
			textColors: [], //corresponds with colors array. null/undefined items will fall back to CSS
			parseDirection: 'x', //which direction to parse the table data
			pieMargin: 20, //pie charts only - spacing around pie
			pieLabelsAsPercent: true,
			pieLabelPos: 'inside',
			lineWeight: 5, //for line and area - stroke weight
			barGroupMargin: 5,
			barMargin: 0.1, //space around bars in bar chart (added to both sides of bar)
		...
		var createChart = {
			...
			bar: function(){
                ...
                var value = tableData.allData();
                var unit = canvas.height()/topValue;
				...
				var xlabelsUL = $('<ul class="visualize-labels-x"></ul>')
					...
					.height(canvas.height())
					.insertBefore(canvas);
                    //.append('<span class="value" style="position:absolute;right:5px;">'+value+'</span>');
                var j = 0;
				$.each(xLabels, function(i){
                    ...
                    if (value[0][j] != 0)
                        {
                        var bot = unit*value[0][j];
                        thisLi.prepend('<span class="value" style="position:absolute;left: 0px;bottom:'+bot+'px;">'+value[0][j]+'</span>');
                        }
                    if (value[1][j] != 0)
                        {thisLi.prepend('<span class="value" style="position:absolute;right: 0px;bottom:'+unit*value[1][j]+'px;">'+value[1][j]+'</span>');}
                    //thisLi.append('<span class="value" style="position:absolute;left:6px;">'+j+'</span>');
                    j++;
