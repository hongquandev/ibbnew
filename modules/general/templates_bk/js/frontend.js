	...
	onBid: function(data) {
		...
		if (json.success == 1 && json.data.property_id) {
            ...
            jQuery(obj).bind('click',function(){
                ...
                var args = {property_id:property_id,
...
*/
	...
	startTimer: function(id) {
		//this._options.timer = setInterval(this.calTime,1000);
            ...
            if (JSONobject.data) {
                for (var property_id in JSONobject.data) {
                    if(JSONobject.data[property_id].stop_bid == 0) {
                        ...
                        if (JSONobject.data[property_id].live_time && JSONobject.data[property_id].live_time <= 0 && self['bid_' + property_id]._options.transfer === false){//BEGIN LIVE
                            showLoadingPopup();
                        ...
                        if ((JSONobject.data[property_id].bidder && JSONobject.data[property_id].bidder.length > 0) || (JSONobject.data[property_id].price && JSONobject.data[property_id].price.length > 0) ) {
                            ...
                            if (JSONobject.data[property_id].isLastBidVendor) {
                                ...
                                if (JSONobject.data[property_id].stop_bid == 1 || parseInt(JSONobject.data[property_id].confirm_sold) == 1) {
                                    ...
                                    jQuery(self['bid_' + property_id]._options.bidder_obj).html('Current Bidder: ' + JSONobject.data[property_id].bidder);
                    ...
                    if (parseInt(JSONobject.data[property_id].confirm_sold) == 1) { // SOLD
                        if(parseInt(JSONobject.data[property_id].rent) == 1){
                            ...
                            jQuery('#mark').attr('src', '../modules/general/templates/images/sold_detail.png');
                        }
                        ...
                        switch (JSONobject.data[property_id].type_property){
                            ...
                            case 'live_auction':
                                    ...
                                    if(self['bid_' + property_id]._options.theblock && ( jQuery('#uniform-step_option').length > 0 || jQuery('#uniform-step_option_'+property_id).length > 0 )){
                                        ...
                                        var options = "";var i_ = 1;var id_op = '#step_option';
                                        var op_ = jQuery('#step_option1').val();
                                        if(typeof op_ == 'undefined' || op_ == null){
                                            op_ = jQuery('#step_option1_' + property_id).val();id_op = '#step_option_' + property_id;
                                        }
                                        var ch = false;var min_ = "";var min_value = "";var bf_ar = [];var af_ar = [];
                                        for(var option in JSONobject.data[property_id].MinMax_options)
                                        {   af_ar.push(option);
                                            if(i_ == 1){
                                                min_ = option;min_value = JSONobject.data[property_id].MinMax_options[option];
                                            }
                                            if(op_ == option ){ch = true;}
                                            i_++;
...
CountDown.prototype = {
   ...
   close:function(){
       ...
       $(".tipsy").remove();
       jQuery(countDown_popup.container).fadeOut('slow',function(){countDown_popup.hide()});
   ...
   setIncrement:function(frm){
         ...
         $.post(url,{'min-incre':$('#min-incre-'+this.property_id,frm).val()
   ...
   resetIncrement:function(){
       ...
       $.post(url,{'min-incre':$('#inc_default_'+this.property_id).val()
               ,'max-incre':0
...
function AddWatermark(img_id,watermark){
    ...
    if(img_id != '')
    {
        ...
        if(watermark == 'OnTheMarket_detail')
        {
