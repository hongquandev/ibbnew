...
function sendEmail_func($from = "", $to = "", $message = "", $subject = "", $ban = '', $from_name = '', $has_content = false, $type = '', $file_att = array())
{
    ...
    /*if ($ban == '') {
        $banner = '<li style="margin-left:0px;list-style-type:none"> <a href=' . ROOTURL . '  target="_blank">
                    <img style="padding-bottom:15px; padding-top:20px;" src=' . ROOTURL . '/modules/general/templates/images/logo-add.png width="280" alt="" /> </a> </li>';
        $mail->Body = emailTemplatesSendBanner($message, $banner);
    } else {
        $count = substr_count($ban, '<li');
        $count_logod = substr_count($ban, 'logo-add.png');
        if ($count < 5 and $count_logod < 1) {
            $ban .= '<li style="margin-left:0px;list-style-type:none"> <a href=' . ROOTURL . '  target="_blank">
                    <img style="padding-bottom:15px; padding-top:15px;" src=' . ROOTURL . '/modules/general/templates/images/logo-add.png width="280" alt="" /> </a> </li>';
        }
        $mail->Body = emailTemplatesSendBanner($message, $ban, true, $to);
    }*/
    ...
    /*SAVE TO DB*/
    include_once ROOTPATH . '/modules/general/inc/email.class.php';
    ...
    $email_data = array(
        ...
        'subject' => $mail->Subject,
        'body' => $mail->Body,
        'to' => serialize($mail->getToAddresses()),
        'cc' => serialize($mail->getCcAddresses()),
        'bcc' => serialize($mail->getBccAddresses()),
        'file_att' => serialize($file_att),
        'scan' => 0,
        'sent' => 0,
        'create_date' => date('Y-m-d H:i:s'),
        'scan_date' => '0000-00-00 00:00:00',
    );
    $email_cls->insert($email_data);
...
function sendEmail_func_cron($data = array())
{
    global $config_cls, $email_log_system;
    require_once ROOTPATH . '/includes/PHPMailerAutoload.php';
    include_once ROOTPATH . '/modules/general/inc/email_log_system.class.php';
    ...
    $mail->addBCC('hongquandev@gmail.com');
    if (is_array($to) && count($to) > 0) {
        $to = array_unique($to);
        foreach ($to as $_to) {
            if(!empty($_to) && strlen($_to) > 0){
                $mail->addAddress($_to);
            }
        }
    }
    ...
    $file_att = unserialize($data['file_att']);
    if (is_array($file_att) && count($file_att) > 0) {
        if (!empty($file_att['path'])) $mail->AddAttachment($file_att['path'], $file_att['filename']);
        if (!empty($file_att['string'])) $mail->AddStringAttachment($file_att['string'], $file_att['filename']);
    }
...
function sendNotificationByEventKey($event_key = '', $params = array(), $variables = array())
{
    ...
    if (!empty($event_key)) {
        ...
        if (isset($params['property_id']) && $params['property_id'] > 0) {
            ...
            $suburb_row = $property_cls->getCRow(array('suburb'),'property_id = '.$property_id);
            $suburb = $suburb_row['suburb'];
            $subject = str_replace(array('[ID]', '[address]', '[detail_link]', '[ROOTURL]','[suburb]'), array($property_id, $address, $detail_link,ROOTURL,$suburb), $subject);
            $email_content = str_replace(array('[ID]', '[address]', '[detail_link]', '[ROOTURL]','[suburb]'), array($property_id, $address, $detail_link, ROOTURL,$suburb), $email_content);
            $sms_content = str_replace(array('[ID]', '[address]', '[detail_link]', '[ROOTURL]','[suburb]'), array($property_id, $address, $detail_link,ROOTURL,$suburb), $sms_content);
            /*Additional Property Notification Contact*/
            $property_agent = A_getAgentManageInfo($property_id);
            if(!is_array($params['to'])){
                $params['to'] = (array) $params['to'];
            }
            if (!empty($property_agent['agent_email']) && !in_array($property_agent['agent_email'], $params['to'])) {
                $params['to'][] = $property_agent['agent_email'];
            }
            $property_row = $property_cls->getCRow(array('nd2_email_address', 'nd2_phone_number'), 'property_id = ' . $property_id);
            if (!empty($property_row['nd2_email_address']) && !in_array($property_row['nd2_email_address'], $params['to'])) {
                $params['to'][] = $property_row['nd2_email_address'];
                $nd2_phone_number = $property_row['nd2_phone_number'];
            }
        }
        ...
        if(!empty($nd2_phone_number)){
            $result['sms'] = sendSMS_func($sms_content,array(0 => $nd2_phone_number),null);
        }
        // LOG EMAIL
...
function sendSMS_func($sms_content,$sms_to,$emails_to){
    ...
    if (!empty($sms_content)) {
        ...
        include_once ROOTPATH . '/modules/general/inc/sms_cron.class.php';
        if (!isset($smscron_cls) || !($smscron_cls instanceof SMSCRON)) {
            $smscron_cls = new SMSCRON();
        }
        $smscron_cls->insert(array('content' => $sms_content, 'to' => serialize((array)$sms_to), 'create_date' => date('Y-m-d H:i:s')));
        if (!$smscron_cls->hasError()) {
            return "OK";
        }
    }
    return false;
