...
try {
    ...
    if ($jsons['isBlock']) {
        ...
        if (is_array($row) and count($row) > 0) {
            if ($row['confirm_sold'] == 1 || $row['stop_bid'] == 1) {
                ...
                out(0, $jsons['msg'], array('in_room_bid' => getParam('in_room_bid'), 'data' => $jsons)); 
            } else {
    ...
    if (in_array($_SESSION['agent']['type'], array('theblock', 'agent')) && $mine == 1) {
                ...
                if ($jsons['isBlock'] && $ok) {
                    if ($step == 0) {
                        if ($increment != $min_incre) {
                            $jsons['error'] = 1;
                            $jsons['msg'] = 'This property is only bid with price is ' . $min_incre . '. Please bid with price is ' . showPrice(($bid_price + $min_incre)) . '.';
                        }
                    } elseif ($step < 0) { //only min_incre
                        if ($increment < $min_incre) {
                            $jsons['error'] = 1;
                            $jsons['msg'] = 'Min increment of this property is ' . showPrice($min_incre) . ' Please bid with price larger than ' . showPrice(($bid_price + $min_incre)) . '.';
                        }
                    } else {
                        if ($increment >= $min_incre && $increment <= $max_incre) {
                            ...
                            $jsons['error'] = 1;
                            if ($min_incre > 0) {
                                $jsons['msg'] = 'Increment of this property between is ' . showPrice($min_incre) . ' and ' . showPrice($max_incre) . '. Please bid with price larger than ' . showPrice(($bid_price + $min_incre)) . ' and smaller than ' . showPrice(($bid_price + $max_incre)) . '.';
                            } else {
                                $jsons['msg'] = 'Max increment of this property is ' . showPrice($max_incre) . ' Please bid with price smaller than ' . showPrice(($bid_price + $max_incre)) . '.';
                            }
                        }
                    }
                }
        ...
        if ($bid_room_cls->isExist($_SESSION['agent']['id'], $property_id)) {
            ...
            $jsons['msg'] = 'It is currently your turn in the autobid queue for this property. If you want to manually bid, please turn off your autobid for this property by clicking on autobid and then the Cancel button.';

            out(0, $jsons['msg'], array('in_room_bid' => getParam('in_room_bid'), 'data' => $jsons));
        }

        $output = Bid_isValid_Mobile($_SESSION['agent']['id'], $property_id, false, false, $_SESSION['agent']['type']);
        if (count($output) > 0) {
            /*$jsons['error'] = 1;
           ...
           $jsons['']*/
            ...
            //get increment price
            $money_step = getParam('money_step', 0); //WHEN BIDDER CHOSE FROM COMBOBOX
            $increment = ($money_step == 0) ? PT_getValueByCode($property_id, 'initial_auction_increments')
                    : $money_step;
            $bid_price = PE_getBidPrice($property_id);
    ...
    out(1, $jsons['msg'], array('in_room_bid' => getParam('in_room_bid'), 'data' => $jsons));
} catch (Exception $e) {
    out('0', $e->getMessage());
}
