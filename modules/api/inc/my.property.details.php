...
for ($i = 0; $i < count($rows); $i++) {
    ...
    if (is_array($_row) and count($_row) > 0) {
        ...
        if (in_array($_row['pay_status'], array(Property::PAY_PENDING, Property::PAY_UNKNOWN))
            || ($_row['auction_sale'] == $auction_sale_ar['auction'] && $_row['start_time'] == '0000-00-00 00:00:00')
        ) { //NOT COMPLETE
            if (Property_isVendorOfProperty($rows[$i]['property_id'], $rows[$i]['agent_id'])) {
            ...
            if ($_row['confirm_sold'] == 1) { //SOLD
                ...
                if ($_row['stop_bid'] == 1) { //SOLD OR PASSEDIN
                    $status = $watermark = $_row['reserve_price'] < $_row['bid_price'] && $_row['reserve_price'] > 0
                            ? 'sold' : 'passed_in';
                    $countdown = $_row['reserve_price'] < $_row['bid_price'] && $_row['reserve_price'] > 0 ? 'SOLD'
                            : 'PASSED IN';
        ...
        if ($status == 'forthcoming') {
            $countdown = $start_time->format($config_cls->getKey('general_date_format'));
        } elseif ($status != 'sale') {
            ...
            if ($isLastBidVendor) {
                $bid_status_bidder = $in_room ? 'In Room Bid' : 'Vendor Bid';
            } else {
                $bid_status_bidder = $_row['stop_bid'] == 1 || $_row['confirm_sold'] == 1 ? 'Last Bidder: ' . $bidder
                        : 'Current Bidder: ' . $bidder;
            }

            $bid_price = showPrice($_row['bid_price']);
            if ($_row['bids'] == 0) {
                $bid_status_price = 'Start Price: ' . $bid_price;
            } else {
                $bid_status_price = $_row['stop_bid'] == 1 || $_row['confirm_sold'] == 1 ? 'Last Bid: ' . $bid_price
                        : 'Current Bid: ' . $bid_price;
            }
        }
    }
    ...
    $rows[$i]['bid_status_bidder'] = $bid_status_bidder;
    $rows[$i]['bid_status_price'] = $bid_status_price;
